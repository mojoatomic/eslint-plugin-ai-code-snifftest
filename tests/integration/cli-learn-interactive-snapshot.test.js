...
describe('CLI learn --interactive (snapshot)', function () {
  ...
  it.skip('happy path: rename constant and map to domain', async function () {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cli-learn-interactive-'));
    
    // Setup test files with recognizable constants
    writeFile(tempDir, 'src/astronomy.js', `
      const isActive = true;
      const hasData = false;
    ...
    const session = await runInteractiveLearn(tempDir, inputs);
    
    // Verify exit code
    assert.strictEqual(session.code, 0, 'Process should exit successfully');
    ...
    const fpPath = path.join(tempDir, '.ai-constants', 'project-fingerprint.js');
    ...
    assert.ok(session.stdout.includes('Naming'), 'Output should show naming prompts');
    assert.ok(session.stdout.includes('Domain-aware constants'), 'Output should show constants section');
  });

  it.skip('back navigation: go back and change decision', async function () {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cli-learn-back-'));
    
    writeFile(tempDir, 'src/test.js', `
      const isReady = true;
      const value1 = 365.25;
      const value2 = 60;
    `);
  ...
  it.skip('skip-all: skip remaining constants', async function () {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cli-learn-skipall-'));
    
    writeFile(tempDir, 'src/constants.js', `
      const isTest = true;
      const val1 = 365.25;
      const val2 = 60;
      const val3 = 1000;
