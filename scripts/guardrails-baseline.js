...
function main() {
  const args = (typeof process !== 'undefined' ? (function(argv){ const out={_:[]}; for(let i=2;i<argv.length;i++){ const a=argv[i]; if(a.startsWith('--')){ const [k,v]=a.split('='); const key=k.replace(/^--/,''); out[key]= v===undefined? true : v; } else { out._.push(a); } } return out; })(process.argv) : {});
  ...
  const currSummary = currRaw && currRaw.categories ? summarize(currRaw) : null;
  const weights = (function(){ const cfg = readJson('.ai-coding-guide.json'); const r=(cfg&&cfg.ratchet)||{}; const d={ weights:{ complexity:10, architecture:8, domainTerms:2, magicNumbers:1 } }; return { ...d.weights, ...(r.weights||{}) }; })();

  if (refactoring && prevBaseline) {
