...
function contextAwareMode({ b, c, baseRaw, currRaw, cfg, refactoring }) {
  const critInc = (cfg.critical || []).some(cat => num(c[cat]) > num(b[cat]));
  ...
  // Structural proxies (rule-level breakdowns)
  ...
  // 3) Density tie-breaker (warn only by default)
  if (currDensity.perK > baseDensity.perK) {
    ...
    // Default: warn, not fail (can be tightened later)
    console.log(lines.join('\n'));
