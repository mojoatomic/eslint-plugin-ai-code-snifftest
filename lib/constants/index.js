...
function allConstants() {
  ...
  for (const [domain, mod] of Object.entries(DOMAINS)) {
    const constantsList = Array.isArray(mod.constants) ? mod.constants : [];
    for (const v of constantsList) {
...
function allTerms() {
  const out = [];
  for (const [domain, mod] of Object.entries(DOMAINS)) {
    const termsList = Array.isArray(mod.terms) ? mod.terms : [];
...
function getDomain(name) {
  ...
  return key && Object.prototype.hasOwnProperty.call(DOMAINS, key) ? DOMAINS[key] : null;
}
