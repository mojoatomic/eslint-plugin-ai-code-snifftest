...
function initCommand(cwd, args) {
  ...
  const enableArch = !args['no-arch'] && args.arch && args.arch !== 'false';
  console.log(`[init] arch flags: no-arch=${String(args['no-arch'])} arch=${String(args.arch)} -> enableArch=${enableArch}`);
  if (enableArch) {
