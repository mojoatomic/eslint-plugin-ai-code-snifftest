...
const { ensureCIWorkflow } = require(path.join(__dirname, '..', '..', 'generators', 'ci-workflow'));
...
if (shouldWriteEslint) writeEslintConfig(cwd, cfg);
  ...
  // Optional: scaffold CI workflow file
  if (args && (args.ci || String(args.ci).toLowerCase() === 'true')) {
    try {
      ensureCIWorkflow(cwd);
      console.log('\nCI workflow generated. Recommended required checks for branch protection:');
      console.log('  - ratchet-and-tests');
      console.log('  - (optional) your primary test job');
    } catch { /* ignore */ }
  }
